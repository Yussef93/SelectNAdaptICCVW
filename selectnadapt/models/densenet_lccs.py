...
class _DenseLayer(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.norm1: LCCS
        self.add_module("norm1", LCCS(num_input_features))
        self.relu1: nn.ReLU
        ...
        self.norm2: LCCS
        self.add_module("norm2", LCCS(bn_size * growth_rate))
        self.relu2: nn.ReLU
...
class _Transition(nn.Sequential):
    def __init__(self, num_input_features: int, num_output_features: int) -> None:
        super().__init__()
        self.add_module("norm", LCCS(num_input_features))
...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        self.features.add_module("norm5", LCCS(num_features))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = self.global_avgpool(out)#F.adaptive_avg_pool2d(out, (1, 1))
...
def _densenet(
    ...
    **kwargs: Any,
    ...
    if pretrained:
        load_url(url=model_urls[arch],progress=progress)
        #_load_state_dict(model, model_urls[arch], progress)

    return model

@BACKBONE_REGISTRY.register()
def densenet121_lccs(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> DenseNet:
